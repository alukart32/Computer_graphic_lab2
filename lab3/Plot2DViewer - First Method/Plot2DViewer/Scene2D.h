...
public:
	...
	void Render(HDC dc) {
		/*
			Отрисовка фигуры
			1) прочесть пару Edges
			2) найти соот. векторы в Vertices
			3) провести линии
			к шагу 1) пока не прочли всю матрицу
		*/
	...
	// вариант через cos sin
	Matrix<> getR(bool direction, int a, int b) {
	...
	void Plot(HDC dc, Func f, bool axes=true)
	{
		...
		//HPEN pen = (HPEN)SelectObject(dc, CreatePen(PS_SOLID,1, RGB(120,120,120)));
