...
LRESULT _stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)		// оконна¤ процедура принимает и обрабатывает все сообщени¤, отправленные окну
{
	...
	case WM_KEYDOWN:
	{
		switch (wParam) {
			...
			// увеличиваем модель (+)
			case VK_OEM_PLUS: 
				scene.model->Apply(Scalling(1 + SCLCOEFF, 1 + SCLCOEFF));
				break;
			// уменьшаем модель (-)
			case VK_OEM_MINUS:
				scene.model->Apply(Scalling(1 - SCLCOEFF, 1 - SCLCOEFF));
			...
			case 0x56:
				...
				// Z - особый случай
			case 0x5A:
				x = scene.model->GetVertexX(scene.model->GetEdgeFirst(11));
				y = scene.model->GetVertexY(scene.model->GetEdgeFirst(11));

				scene.model->Apply(Translation(x, y)*Mapping(1)*Translation(-x, -y));
				break;
				// Q
				case 0x51:
			//	double a_x = scene.model->GetVertexX(scene.model->GetEdgeFirst(1));
			//	double a_y = scene.model->GetVertexY(scene.model->GetEdgeFirst(1));

			//	// получение коорд. второй точки
			//	double b_x = scene.model->GetVertexX(scene.model->GetEdgeSecond(3));
			//	double b_y = scene.model->GetVertexY(scene.model->GetEdgeSecond(3));

				x = scene.model->GetVertexX(scene.model->GetEdgeFirst(2));
				y = scene.model->GetVertexY(scene.model->GetEdgeFirst(2));

				// T R M R T
				scene.model->Apply(Translation(x, y)*scene.getR(true, 1, 2)*Mapping(-1)*scene.getR(false, 1, 2)*Translation(-x, -y));
				break;
		}
